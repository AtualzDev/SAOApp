        ...
        <div className="fixed inset-0 z-[60] flex justify-end">
            ...
            <div className="relative w-full max-w-2xl bg-[#F8FAFC] h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                ...
                <div className="bg-white px-8 py-6 border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                    <h2 className="text-xl font-bold text-slate-800">Detalhes do Lançamento</h2>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-slate-100">
                            <span className="text-xs font-semibold text-slate-400 block mb-1">Recebimento</span>
                            <span className="text-base font-bold text-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-white p-5 rounded-xl border border-slate-100">
                            <h3 className="font-bold text-slate-700 mb-1 text-lg">{transaction.fornecedor || 'Fornecedor não informado'}</h3>
                            <p className="text-sm text-slate-500 font-medium">00.000.000/0001-00</p>
                            <p className="text-sm text-slate-400 mt-2">Endereço não cadastrado</p>
                        </div>

                        {/* Instituição */}
                        <div className="bg-white p-5 rounded-xl border border-slate-100">
                            <h3 className="font-bold text-slate-700 mb-1 text-lg">{transaction.instituicao_beneficiada || 'CAPEC'}</h3>
                            <p className="text-sm text-slate-400 mt-2">Nova Iorque, NY, EUA</p>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100">
                                        ...
                                        <th className="px-6 py-3 text-xs font-bold text-slate-400 uppercase text-right">VALOR TOTAL</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                    {transaction.items?.map((item, index) => (
                                        <tr key={item.id || index}>
                                            ...
                                            <tr>
                                                <td colSpan={5} className="p-6 text-center text-slate-400">Nenhum item encontrado neste lançamento.</td>
                                            </tr>
                                        )}
                                </tbody>
                            </table>
